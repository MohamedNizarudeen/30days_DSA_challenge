class Minimumremovals 
{
	public static int findMin(int[] arr)
	{
		// Stores the length of the maximum size sub-array that satisfies the given condition
		int max_range = 0;
		...
		for (int i = 0; i < arr.length && arr.length - i > max_range; i++)
		{
			...
			*/
			...
			// Find the maximum size sub-array `arr[i..j]` that satisfies the above invariant
			for (int j = i; j < arr.length; j++)
			{
				// Find the min and max elements in the current sub-array.
				// This must be done in constant time.
